...
__p+='`,r},lazyExec:function(t,e,r,a){const i=this,s=i.attacher;s&&e.eventArray.forEach(function(l,g){const f=a.querySelector(`[data-co-event="${g}"]`);if(!f){n.debug&&console.warn(`Event target not found for ID ${g} in template ${r.tmplId}`);return}delete f.dataset.coEvent;for(let m=0,d=l.length;m<d;m++){const h=l[m];h.eventFunc&&(Array.isArray(h.eventFunc)?h.eventFunc.forEach(function(y){s(i,t,e,r,a,f,y,h)}):s(i,t,e,r,a,f,h.eventFunc,h))}})},trigger:function(t,e){const r=new Event(e,{bubbles:!0,cancelable:!0});t.dispatchEvent(r)},attacher:function(t,e,r,a,i,s,l,g){const f=t.trigger,m=H(i),d=U(i)===1?m:null;if(!l)return;const h=[s,null,{data:e,customData:g.customData,element:s,componentElement:d||(m==null?void 0:m.parentElement),component:a,compomint:o}];if(typeof l=="function"){const k=function(E){E.stopPropagation(),h[1]=E;try{l.call(...h)}catch(b){if(console.error(`Error in event handler for template ${a.tmplId}:`,b,l),n.throwError)throw b}};s.addEventListener("click",k),g.element=s,g.eventFunc=k;return}if(!ee(l))return;const y=l,v=y.triggerName;v&&(a.trigger=a.trigger||{},a.trigger[v]={}),Object.keys(y).forEach(function(k){const E=y[k];if(k==="load"){h[1]=s;try{E.call(...h)}catch(w){if(console.error(`Error in 'load' event handler for template ${a.tmplId}:`,w,E),n.throwError)throw w}return}else if(k==="namedElement"){a[E]=s;return}else if(k==="triggerName")return;const b=function(w){w.stopPropagation(),h[1]=w;try{E.call(...h)}catch(N){if(console.error(`Error in '${k}' event handler for template ${a.tmplId}:`,N,E),n.throwError)throw N}};s.addEventListener(k,b),g.element=s,l[k]=b,v&&f&&(a.trigger[v][k]=function(){f(s,k)})})}},element:{pattern:/##%([\s\S]+?)##/g,exec:function(t){const e=t.split("::");return`';
(() => {
...
${i}
  ...
  `);const n=["en","ko","ja","zh"];function o(){let t;try{t=localStorage.getItem("compomint-lang")}catch(a){console.warn("Failed to read language preference from localStorage:",a)}if(!t){const a=navigator.language.split("-")[0];t=n.includes(a)?a:"en"}const r=new URLSearchParams(window.location.search).get("lang");r&&n.includes(r)&&(t=r),document.documentElement.lang=t,setTimeout(function(){try{typeof window.updateMetaTags=="function"&&window.updateMetaTags()}catch(a){console.warn("Failed to update meta tags:",a)}},100)}o()});document.addEventListener("DOMContentLoaded",async function(){const n=c.addTmplByUrl(["templates/layout.cmint","templates/header.cmint","templates/hero.cmint","templates/features.cmint","templates/vscode-extension.cmint","templates/examples.cmint","templates/documentation.cmint","templates/footer.cmint","templates/ui-components.cmint","templates/ui.components.cmint","templates/pg.components.cmint","templates/demo-components.cmint"]);n?n.then(()=>{console.log("loaded templates, now initialize app"),me()}):(console.error("Failed to load templates, cannot initialize app"),document.getElementById("app-container").innerHTML='<div class="text-center p-8"><h1 class="text-2xl text-red-600">Error loading application templates</h1></div>'),de()});function de(){let n;try{n=localStorage.getItem("compomint-theme")}catch(e){console.warn("Couldn't access localStorage:",e)}const o=window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches;n==="dark"||n===null&&o?(document.documentElement.classList.add("dark"),document.body.classList.add("dark-mode")):(document.documentElement.classList.remove("dark"),document.body.classList.remove("dark-mode"))}async function me(){const n={lightweight:`<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"></path>
    </svg>`},o=j.ui.Header({menuItems:[{label:c.i18n.footer.links.home("Home"),url:"#home",active:!0},{label:c.i18n.footer.links.features("Features"),url:"#features"},{label:c.i18n.vscode.title("VSCode"),url:"#vscode-extension"},{label:c.i18n.footer.links.examples("Examples"),url:"#examples"},{label:c.i18n.footer.links.docs("Documentation"),url:"#documentation"}]}),t=j.ui.Hero({title:c.i18n.app.title("How to Create Web Components Easily"),subtitle:c.i18n.app.subtitle("Compomint is a lightweight JavaScript framework that provides a template-based component system."),primaryButtonText:c.i18n.app.getStarted("Get Started"),primaryButtonUrl:"#documentation",secondaryButtonText:"GitHub",secondaryButtonUrl:"https://github.com/kurukona/compomint",codeExample:`<template id="hello-world">
...
document.body.appendChild(button.element);`},{class:"h-[700px]",interactive:!0,title:c.i18n.examples.stateManagement.title("State Management"),description:c.i18n.examples.stateManagement.description("How to manage internal component state and respond to events"),type:"codeIsTemplateFile",template:await(await fetch("templates/demo/ui.Counter.cmint")).text(),code:`// Create a counter component for an example
  const counter = tmpl.ui.Counter({
    initialCount: 1,
  });
  document.body.appendChild(counter.element);`},{class:"h-[1000px]",interactive:!0,title:c.i18n.examples.complexComponent.title("Complex Component"),description:c.i18n.examples.complexComponent.description("A more complex component example: Todo List"),type:"codeIsTemplateFile",template:await(await fetch("templates/demo/ui.TodoList.cmint")).text(),code:`// Create a todo list component for an example
  ...
  document.body.appendChild(todoList.element);`},{class:"h-[1400px]",interactive:!0,title:c.i18n.examples.multiTemplate.title("Multi-Template Application"),description:c.i18n.examples.multiTemplate.description("Complete application example combining multiple templates"),type:"codeIsTemplateFile",template:await(await fetch("templates/demo/ui.UserManagement.cmint")).text(),imports:[],code:`// Create a user management system for an example
...
document.body.appendChild(userManagement.element);`}]}),s=j.ui.Documentation({title:c.i18n.app.docTitle("Learn More"),description:c.i18n.app.docDescription("Check out detailed documentation and resources for Compomint."),links:[{label:"Getting Started",url:"https://kurukona.github.io/compomint/"},{label:"Basic Usage",url:"https://kurukona.github.io/compomint/#basic-usage"},{label:"Template Syntax",url:"https://kurukona.github.io/compomint/#template-syntax"},{label:"API Reference",url:"https://kurukona.github.io/compomint/#api-reference"},{label:"Examples",url:"https://github.com/kurukona/compomint/tree/master/examples"},{label:"GitHub",url:"https://github.com/kurukona/compomint"}]}),l=j.ui.Footer({description:c.i18n.footer.description("Compomint is a lightweight JavaScript framework for creating web applications with a component-based architecture."),links:[{label:c.i18n.footer.links.home("Home"),url:"#home"},{label:c.i18n.footer.links.features("Features"),url:"#features"},{label:c.i18n.footer.links.examples("Examples"),url:"#examples"},{label:c.i18n.footer.links.docs("Documentation"),url:"#documentation"},{label:"GitHub",url:"https://github.com/kurukona/compomint"}],email:"choish@kurukona.com",github:"kurukona/compomint",year:new Date().getFullYear(),copyright:"Compomint"}),g=j.app.Layout({header:o,hero:t,features:e,vscodeExtension:a,examples:i,documentation:s,footer:l});document.getElementById("app-container").appendChild(g.element),document.querySelectorAll('a[href^="#"]').forEach(m=>{m.addEventListener("click",function(d){d.preventDefault();const h=this.getAttribute("href");if(h==="#")return;const y=document.querySelector(h);console.log("Target ID:",h,"Target Element:",y),y?window.scrollTo({top:y.offsetTop-70,behavior:"smooth"}):console.warn("Target element not found:",h)})})}
